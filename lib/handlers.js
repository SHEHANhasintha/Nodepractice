...
handlers.accountCreate =  function(data,callback){
  ...
  //reject andy request where it is not the method get
  if (data.method == 'get'){
    // TODO: define what fwe need specially for index.html
    let templateData = {
...
handlers._users.post =  function(data,callback){
  //validate data @PARAM: FirstName Lastname phoneNumber password
    ...
    tosAgreement = (typeof(data.payload.tosAgreement) == 'boolean' || data.payload.tosAgreement == "agree") ? true : false;
              ...
              dataLib.read('users',phoneNumber,function(err,data){
                if(err){
                  ...
                  dataLib.create('users',phoneNumber,data,function(err){
                    if(!err){
                      callback(200,{"success" : "success"},"json");
                    }else{
...
handlers._checks.delete = function(data,callback){
  //validation check
  let checkId = typeof(data.payload.uid) == 'string' && data.payload.uid.trim().length == handlers.defaultTokenLength ? data.payload.uid : false;
